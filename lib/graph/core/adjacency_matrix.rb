class AdjacencyMatrix
  ...
  def initialize(vertices)
    raise ArgumentError.new('Argument must be a list of vertices') if vertices.any?{|v| !v.instance_of?(Vertex)}
  ...
  def add_edge(initial_vertex_index, final_vertex_index)
    raise ArgumentError.new('index must be a number') unless initial_vertex_index.instance_of?(Integer) && final_vertex_index.instance_of?(Integer)
    raise ArgumentError.new('index must be inside matrix size') if initial_vertex_index > vertices.size || final_vertex_index > vertices.size
